<script setup lang="ts">
	...
	**********************************************************/	
	...
	const submitData = () => {
		emits('setList', arrChklist.value)
	}
...
<div class="w-full h-[calc(100vh_-_360px)] mt-2 border-2 border-gray-400 overflow-x-hidden overflow-y-auto">
	<div class="w-full h-12 pt-3 text-white text-center bg-violet-800 relative">
		...
		<div v-if="liwaData.length > 0" class="h-8 px-2 py-[0.125rem] absolute top-1 right-2 rounded-md bg-orange-400 text-white" @click="submitData()">上傳資料</div>
	</div>
