<script setup lang="ts">
	...
	**********************************************************/	
	...
	const saveData = async (idx) => {
	    ...
	    if (!msg) {
	    	if (action.value == 'add') {
	    		liwaData.value[idx].item1 = data.value.key
	    	}
			liwaData.value[idx].item1 = liwaDetail.value.item1
			liwaData.value[idx].item2 = liwaDetail.value.item2
	...
	const setMainID = (idx) => {
		...
		liwaDetail.value.item1 = liwaData.value[idx].item1
		liwaDetail.value.item2 = liwaData.value[idx].item2
	...
	const itemDelete = async (idx) => {
		...
		let keydata = {
			"action":action.value,
			"JWT":window.localStorage.getItem('liwaJWT'),
			"item1":liwaData.value[idx].item1
		}
		let datastr = JSON.stringify(keydata)
	    const useMyFetch = createFetch({
	      baseUrl: APIsvr.value,
...
<div class="w-full h-auto bg-gray-100">
	<div class="w-full h-full mx-auto my-0 bg-white relative border-2 border-slate-800">
		<!-- 先設定 Title & btnAdd -->
			...
			<table class="w-full divide-x divide-y divide-gray-200 bg-white ">
				<thead>
					<tr class="w-full bg-emerald-500">
						<th v-for="(thead, index) in state.headCol" class="w-1/2 text-white py-4">{{ thead }}</th>
					</tr>
				</thead>
				<tbody>
					<tr class="w-full relative odd:bg-white even:bg-slate-200"
						...
						:data-id="item.item1"
					>
						<td scope="row" class="w-1/2 pt-3 pl-2 border-b-2 border-b-slate-300 ">
						...
						<td scope="row" class="w-1/2 pt-3 pl-2 border-b-2 border-b-slate-300 ">
							<div class="w-full h-full flex flex-row justify-between">
								<div class="w-full h-12 pl-4 relative" 
									@click="setMainID(index)">
									{{ item.item2 }}
								</div>
