<script setup lang="ts">
	...
	**********************************************************/
	...
	const fetchData = async () => {
		...
		let url = `${APIsvr}/041_havelist.php?siteID=${siteID}&userID=${userID}&filterName=${filters.value.username}&filterMobile=${filters.value.mobile}&filterRole=${filters.value.role}&filterUniqID=${filters.value.uniqID}&filterIsFirm=${filters.value.isFirm}&orderCol=${orderCol.value}&sortDir=${sortDir.value}&page=${page.value}&pageSize=${pageSize.value}`
	...
	const saveFilters = () => {
		...
		window.sessionStorage.setItem('liwafilter_041', datastr)
	}

	const refetchFilters = () => {
		let tmpFilters = window.sessionStorage.getItem('liwafilter_041')
		if ((tmpFilters == "") || (tmpFilters == null)) {
			// sessionStorage內的liwafilters有值
			filters.value.username = ''
			filters.value.mobile = ''
			filters.value.role = ''
			filters.value.uniqID = ''
			filters.value.isFirm = ''
	...
	const fullPicpath = computed(() => {
		return Imgsvr.value
	})

	onMounted(() => {
    	let compName = window.sessionStorage.getItem('liwaSiteName')
    	const title = useTitle(compName+`- 會員列表`)
			        	...
			        	<table class="min-w-full divide-x divide-y divide-gray-200 bg-white">
			            	<thead class="bg-gray-50">
			                	<tr class="bg-emerald-300">
			                  		<th class="thPanel w-1/12">
											<div class="w-8 h-8 border-4 border-slate-500 pl-4" @click="toggleChkAll()">
			                    	...
			                    	會員資格
			                  		...
			                  		<th scope="col" v-if="useShowmode()>2" class="thPanel w-2/12">
			                    	身份證字號/統一編號
			                  		</th>
			                  		<th scope="col" v-if="useShowmode()>2" class="thPanel w-3/12">
			                    	積分/等級
			                  		</th>
			                	</tr>
			              	</thead>
							<tbody class="w-full bg-white ring-1 ring-gray-50">
								<tr 
							        ...
							        <td scope="col" class="tdPanel w-2/12" @click.prevent="setMainID(user.mainID)">
							        	<img :src="Imgsvr+user.picpath" width="40" class="float-left" /><span class="text-md pl-2 mt-2" style="line-height:2.75rem">{{ user.memberNM }}</span>
							        </td>
							        <td v-if="useShowmode()>1" class="tdPanel w-2/12" @click.prevent="setMainID(user.mainID)">{{ user.mobile }}
							        </td>
						            <td v-if="useShowmode()>1" class="tdPanel w-2/12" @click.prevent="setMainID(user.mainID)">{{ user.roles }}
						            </td>
						            <td v-if="useShowmode()>2" class="tdPanel w-2/12" @click.prevent="setMainID(user.mainID)">{{ user.uniqID }}
						            </td>
						            <td v-if="useShowmode()>2" class="tdPanel w-3/12" @click.prevent="setMainID(user.mainID)">{{ user.scores }} / {{ user.grade }}
						            </td>  
								</tr>
							</tbody>
			            </table>
