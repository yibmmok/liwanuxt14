<script setup lang="ts">
	/*********************************************************
	prog name: 郵件列表, author: James Lin, date: 2022/09/05
	主要功能: 顯示所有郵件的列表及刪除程式 
	**********************************************************/
	...
	const mainID = ref('')
	const progID = ref('A13')
	const APIsvr = ref('')
	// const liwaDetail1 = ref([])  // 搜尋對話盒的郵件類別過濾
	...
	// 	let url = `${APIsvr.value}/A13_haveItemType.php?${sQuery}`
...
<div v-if="isGrid" class="w-full h-[calc(100vh_-_170px)] relative">
	<div class="w-full mx-auto">
		<liwaGrid
			:tblTitle="'郵件列表'"
			:progID="'A13_M'"
			:viewUrl="'A13_havelist.php'"
			:dataUrl="'A13_edit.php'"
			:params="params"
			:page="page"
			:pageSize="pageSize"
			@sendMsg="sendMsg"
	    ...
	    <div class="flex justify-center w-full h-screen bg-transparent items-start antialiased">
	      	<div class="h-full lg:h-[calc(100%_-_28rem)] flex flex-col mt-4 w-11/12 sm:w-5/6 lg:w-1/2 max-w-lg mx-auto rounded-lg border border-gray-300 shadow-xl">
	        	<div class="relative flex flex-row justify-between px-6 py-2 bg-white border-b border-gray-200 rounded-tl-lg rounded-tr-lg text-center ">
	        		<div class="w-10/12 h-8 text-2xl text-center">郵件查詢</div>
	        		<div class="w-2/12 h-8 flex flex-row justify-between">
